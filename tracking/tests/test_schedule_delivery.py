...
def test_setup_bootstrap_does_not_suppress_first_real_scheduled_report(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    task_module.bootstrap_user_data(user.id)
    ...
    task_module.run_user_report.run(user.id)
...
def test_tick_due_schedules_enqueues_due_job(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    enqueued_calls: list[int] = []
    monkeypatch.setattr(task_module.run_user_report, "delay", lambda user_id: enqueued_calls.append(user_id))

    enqueued = task_module.tick_due_schedules()

    schedule = Schedule.objects.get(user=user)
    assert enqueued == 1
    assert enqueued_calls == [user.id]
    assert schedule.next_run_at == due_at + timedelta(days=1)
